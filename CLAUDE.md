...
On top of all three sits the **Ruhemodus**: [RestMode.tsx](src/components/RestMode.tsx) turns a running rest into a state of its own — full screen, paper ground, the time huge, a bar that fills as the pause runs down, "Danach: Satz 2 · 82,5 kg × 5", and two actions (+30 s, Weiter). During a rest the phone lies on the floor or the bench, and from a metre away the number in the bar at the bottom edge is unreadable.
